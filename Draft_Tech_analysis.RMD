...
treefitRpart <- rpart(log(mental_health_days) ~ inequality + median_inc + hs_grad + college + unempl + child_poverty + single_parent + severe_housing + food_index + mh_providers + pop_provider_ratio, data=ranked, control = list(maxdepth = 20, cp=0.009)) 
...
```{r}
#predict(treefitRpart)

#identify best cp value to use
best <- treefitRpart$cptable[which.min(treefitRpart$cptable[,"xerror"]),"CP"]

#produce a pruned tree based on the best cp value
pruned_tree <- prune(treefitRpart, cp=best)
