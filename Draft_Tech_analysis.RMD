...
# put salary on log scale and fit reg. | distress rate
treefit <- tree(log(mental_distress_rate) ~ inequality + median_inc + child_poverty, data=ranked)
summary(treefit)
...
```{r}
#treefitRpart <- rpart(log(mental_distress_rate) ~ inequality + median_inc + child_poverty, data=ranked, control = list(maxdepth = 20, cp=0.009)) # 8 terminal/leaf nodes, but slightly different than the nodes from the tree function above. 

model1 <- rpart(mental_health_days ~ inequality + median_inc + hs_grad + college + unempl + child_poverty + single_parent + severe_housing + food_index + mh_providers + pop_provider_ratio, data=ranked, control = list(maxdepth = 11, cp=0.009)) 

#treefitRpart <- rpart(log(mental_distress_rate) ~ inequality + median_inc + child_poverty, data=ranked, control=rpart.control(cp=.0001)) 
...
tree <- tree(mental_health_days ~ inequality + median_inc + hs_grad + college + unempl + child_poverty + single_parent + severe_housing + food_index + mh_providers + pop_provider_ratio, data=ranked, control = list(maxdepth = 8, cp=0.009)) # Fits tree 

prune.rpart(treefitRpart, best=5, cp=0.009) # Returns best pruned tree 

prune.tree(tree, best=5, cp=0.009)
#prune.rpart(treefitRpart,best=5, newdata=ranked, cp=0.009)  # no test data set, so use training data

my.tree.seq = prune.tree(tree, cp=0.009) # Sequence of pruned 

#my.tree.seq = prune.rpart(treefitRpart, cp=0.009) # Sequence of pruned 
# tree sizes/errors
...
'''
split_dummy <- sample(c(rep(0, 0.7 * nrow(ranked)),  # Create dummy for splitting
                        rep(1, 0.3 * nrow(ranked))))
split_dummy                                        # Print dummy


data_train <- ranked[split_dummy == 0, ]

data_test <- ranked[split_dummy == 1, ]    
...
sample = sample.split(ranked, SplitRatio = 0.8) # splits the data in the ratio mentioned in SplitRatio. After splitting marks these rows as logical TRUE and the the remaining are marked as logical FALSE
train_set1 =subset(ranked, sample ==TRUE)# creates a training dataset named train1 with rows which are marked as TRUE

test_set1=subset(ranked, sample==FALSE)
...
```{r}
model2 <- tree(mental_health_days ~ inequality + median_inc + hs_grad + college + unempl + child_poverty + single_parent + severe_housing + food_index + mh_providers + pop_provider_ratio, data=train_set1)
...
prune.tree(model2, best=5)

prune.tree(model2, best=5, newdata=test_set1, method = "deviance")

# Sequence of pruned tree sizes/errors
model2_tree_seq = prune.tree(model2) 
plot(model2_tree_seq) # error versus plot size
# Vector of error rates 
# for prunings, in order 
deviance1 <- model2_tree_seq$dev
deviance1

pchisq( deviance1[ length(deviance1) ], length(test_set1$mental_health_days)-1 , lower.tail = F )
pchisq( deviance1[ length(deviance1)-6 ], length(test_set1$mental_health_days)-7 , lower.tail = F )

#Predicting Model2

p1<-predict(model2, ranked)

mae(p1, ranked$mental_health_days)
mape(p1, ranked$mental_health_days)
...
'''
model3 <- rpart(mental_health_days ~ inequality + median_inc + hs_grad + college + unempl + child_poverty + single_parent + severe_housing + food_index + mh_providers + pop_provider_ratio, data=test_set1, control = list(maxdepth = 11, cp=0.009))
...
'''
set.seed(123)   #  set seed to ensure you always have same random numbers generated
sample = sample.split(ranked, SplitRatio = 0.5) # splits the data in the ratio mentioned in SplitRatio. After splitting marks these rows as logical TRUE and the the remaining are marked as logical FALSE
train_set1 =subset(ranked, sample ==TRUE)# creates a training dataset named train1 with rows which are marked as TRUE
...
#Regression
#set.seed(123)
#train = sample(1:nrow(ranked), nrow(ranked)/2)
...
#model_ment_helth_day_tree <- tree(mental_health_days ~ inequality + median_inc + hs_grad + college + unempl + child_poverty + single_parent + severe_housing + food_index + mh_providers + pop_provider_ratio, ranked, subset=train)


model_ment_helth_day_tree <- rpart(mental_health_days ~ inequality + median_inc + hs_grad + college + unempl + child_poverty + single_parent + severe_housing + food_index + mh_providers + pop_provider_ratio, ranked, subset=train, control = list(maxdepth = 13, cp=0.009))


summary(model_ment_helth_day_tree)


rpart.plot(model_ment_helth_day_tree)
fancyRpartPlot(model_ment_helth_day_tree)
#plot(model_ment_helth_day_tree)
#text(model_ment_helth_day_tree,pretty=0) 


printcp(model_ment_helth_day_tree)
#cv_ment_helth_day=cv.tree(model_ment_helth_day_tree)

plotcp(model_ment_helth_day_tree)
#plot(cv_ment_helth_day$size,cv_ment_helth_day$dev, type='b')
...
#prune_model_ment_helth_day_tree=prune.tree(model_ment_helth_day_tree, best=5)

summary(prune_model_ment_helth_day_tree)

printcp(prune_model_ment_helth_day_tree)
#cv_ment_helth_day=cv.tree(model_ment_helth_day_tree)

plotcp(prune_model_ment_helth_day_tree)
#plot(cv_ment_helth_day$size,cv_ment_helth_day$dev, type='b')


fancyRpartPlot(prune_model_ment_helth_day_tree)
#plot(prune_model_ment_helth_day_tree)
#text(prune_model_ment_helth_day_tree,pretty=0)
...
set.seed(123)
bag_ment_hlth_days=randomForest(mental_health_days ~ inequality + median_inc + hs_grad + college + unempl + child_poverty + single_parent + severe_housing + food_index + mh_providers + pop_provider_ratio, data=ranked, subset=train, mtry=11, importance=TRUE)

bag_ment_hlth_days

#TPredict/est Random Forest Model
