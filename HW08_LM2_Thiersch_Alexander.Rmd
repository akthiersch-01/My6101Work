...
The correlation matrices between Pearson and Spearman are relatively similar. However, The Spearman's correlation matrix does display correlation coefficients that are overall larger that the Pearson'correlation matrix.
...
table1$coefficients %>%
  kbl(caption="Baseline Linear Model: lm(Tuser~Humidity)",
...
table2$coefficients %>%
  kbl(caption="Linear Models for Num & Cat predictors: lm(Tuser~Humidity+Weather)",
       format= "html", col.names = c("Estimate", "Std. Error", "t-value", "Pr(>|T|)"),
      align="r") %>%
   kable_classic_2(full_width = F, html_font = "helvetica")
...
table3$coefficients %>%
  kbl(caption="Linear Model w/ Interaction Term: lm(Tuser~Humidity:Weather)",
       format= "html", col.names = c("Estimate", "Std. Error", "t-value", "Pr(>|T|)"),
      align="r") %>%
   kable_classic_2(full_width = F, html_font = "helvetica")
...
a<-aov(Tusers~Humidity, data = bike_final)

TukeyHSD(a, ordered = FALSE, conf.level=0.95)
...
b<-aov(Tusers~Weather, data = bike_final)
TukeyHSD(b)
